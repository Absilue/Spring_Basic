        ...
        <c:if test="${not empty searchResult}">
            <c:forEach var="museum" items="${searchResult}">
                <div class="post-container">
                    <div class="post-info">
                        <div>
                            미술관명 : <a href="list2?museum_Id=${museum.museum_Id}">${museum.name}</a> <br>
                            미술관 위치 : ${museum.location} <br>
                            미술관 운영시간 : ${museum.time_open} - ${museum.time_close} <br>
                            미술관 휴관일 : ${museum.closure} <br>
                            미술관 전화번호 : ${museum.tel} <br>
                        </div>
                        <img src="resources/img/${museum.img}" alt="${museum.name} 이미지">
                    </div>
                    <hr color="#e75480">
                </div>
            </c:forEach>
